...
//main program that generates the password's validity
var passwordCheckOld = function () 
{
    var stringPassword, arrayBadPasswords, stringOutput; //this line declares all of the variables
    ...
    //above is the full list of invalid passwords
    
    stringOutput = "";
       if (stringPassword.length <8) //ensures the user's password is greater than 8 chracters
    {
      stringOutput= "That password is less than 8 characters. Please select a different password that is more than 8 characters."; //error message if the password is less than 8 chracters
    }
    else
    {
      for (var i=0; i<arrayBadPasswords.length; i++) //checks validity of the password
      {
          if (stringPassword === arrayBadPasswords[i]) //ensures the password is not on the invalid password list
          {
           stringOutput = "This password is invalid. Please try a different password."; //error message if the password is on the invalid password list
           break;
          }
      }
    }
    ...
    {
      $("output").value = "That's a valid password."; //message once the user inputs a password that is greater than 8 characters and not on the invalid password list
    }
};
...
{
//    $("password").value = "";
   $("password").focus(); //generates check
   $("check_password").onclick = passwordCheckOld; //checks password when button is clcked
};